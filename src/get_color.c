...
/*   Updated: 2024/03/11 22:05:09 by shhuang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

t_hit	hit_shape(t_shape	*obj, t_camera *camera, t_cyl_utils *data,
			t_vec3 *raydir)
{
	t_hit	rayhit;
	...
	rayhit = (t_hit){0};
	if (obj->type == Sphere)
		rayhit.t = hit_sphere(&obj->shape.sphere,
				(t_vec3){camera->x, camera->y, camera->z}, raydir);
	else if (obj->type == Plane)
		rayhit.t = hit_plane(&obj->shape.plane,
				(t_vec3){camera->x, camera->y, camera->z}, raydir);
	else if (obj->type == Cylinder)
		rayhit.t = hit_cyl(&obj->shape.cylinder,
				(t_vec3){camera->x, camera->y, camera->z}, raydir, data);
